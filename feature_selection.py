...
def plot_feature_importance(scores, top_n=20):
    ...
    plt.title('Top {} Features by ANOVA F-score'.format(top_n))
    plt.xlabel('ANOVA F-score')
    
    # Plot Mutual Information scores
    plt.subplot(2, 1, 2)
    sns.barplot(data=scores.head(top_n), x='Mutual_Info_Score', y='Feature')
    plt.title('Top {} Features by Mutual Information'.format(top_n))
...
def main():
    ...
    # Save results to CSV
    feature_scores.to_csv('data/feature_scores.csv', index=False)
    print("\nFeature scores saved to 'data/feature_scores.csv'")
    
    print("\nGenerating feature importance plots...")
    plot_feature_importance(feature_scores)
    print("Plots saved to 'data/feature_importance.png'")
